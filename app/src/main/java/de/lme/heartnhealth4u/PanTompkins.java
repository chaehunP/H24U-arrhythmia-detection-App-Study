...
public class PanTompkins extends LmeFilter
{
	...
	public int					samplingRate	= 200;
	/** sampling time in ms */
	public float				samplingTime	= 5f;
	/** relative time counter since creation of this instance */
	// public long runtimeCounter = 0;
	...
	public static class QRS
	{
		...
		public double maxCorr (QRS other)
		{
			...
			for (_n = -8; _n < 8; ++_n)
			{
				...
				for (_i = 0; _i < values.num; ++_i)
				{
					...
					_y = (other.values.getIndirect( _n + _i ) - other.mean);
		...
		public void drawBeat (SamplingPlot plot)
		{
			// plot last qrs
			for (int i = 0; i < values.num; ++i)
			{
				plot.addValue( values.values[ i ], i );
				...
				// .getValueHead()
				// - PanTompkins.QRS.qrsCurrent.values.num
				// + i,
				// new PlotMarkerDefault( DefaultMark.LINE_VERTICAL,
				// null, 1f ) );

				// ((SamplingPlot) m_plots[ PLOT_LIVE ]).setMarker( ((SamplingPlot) m_plots[ PLOT_LIVE
				// ]).getValueHead()
				// - m_pants.qrsCurrent.values.num + i - m_pants.totalDelay, new PlotMarkerDefault(
	 ...
	 * 
	 */
	public PanTompkins (int samplingRateInHz)
	{
		...
		Log.d( "lme.pants", "sampling: " + samplingRateInHz + "  wndLength: " + wndLength );
	...
	public double next (double xnow, long timestamp)
	{
		...
		if (startProcessing <= 0)
		{
			...
			if (lastBeat > 3500)
			{
				...
				QRS.qrsCurrent.classification = QRS.QrsClass.VIRTUAL;
				QRS.qrsCurrent.arrhythmia = QRS.QrsArrhythmia.CARDIAC_ARREST;
				...
				if (lastBandPeak >= postSegment)
				{
					...
					if (QRS.template1.classification == QRS.QrsClass.INVALID || QRS.template2.classification == QRS.QrsClass.INVALID)
					{
						...
						if (beatCounter == 6)
						{
							...
							for (int i = 0; i < 6; ++i)
							{
								qrsRefTemp = (QRS) qrsHistory.getPastValue( i );
							...
							for (int i = 0; i < 3; ++i)
							{
								qrsRefTemp = (QRS) qrsHistory.getPastValue( sortList.get( i ) );
